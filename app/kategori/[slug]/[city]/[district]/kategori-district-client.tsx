...
export default function KategoriDistrictClient({ slug, katName, city, cityName, district, districtName }: {
  slug: string; katName: string; city: string; cityName: string; district: string; districtName: string
}) {
  const [businesses, setBusinesses] = useState<any[]>([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    fetch(API + '/api/businesses?categorySlug=' + slug + '&city=' + encodeURIComponent(cityName) + '&district=' + encodeURIComponent(districtName) + '&limit=20&sortBy=trustScore')
      .then(r => r.json())
      .then(d => { setBusinesses(d.businesses ?? d.data ?? []); setLoading(false) })
      .catch(() => setLoading(false))
  }, [slug, cityName, districtName])
      ...
      <div className="min-h-screen bg-[#0A0A0A]">
        <div className="bg-gradient-to-b from-[#1a1a2e] to-[#0A0A0A] px-4 pt-8 pb-6">
          <div className="flex items-center gap-2 text-white/40 text-sm mb-3 flex-wrap">
            ...
            <ChevronRight size={14} />
            <Link href={'/kategori/' + slug + '/' + city} className="hover:text-white">{cityName}</Link>
            <ChevronRight size={14} />
            <span className="text-white">{districtName}</span>
          </div>
          <h1 className="text-2xl font-bold text-white mb-1">{districtName} {katName}</h1>
        ...
        <div className="px-4 pb-8">
          ...
          {loading ? (
            ...
            <div className="text-center py-12 text-white/30">
              ...
              <p>Bu bolgede isletme bulunamadi</p>
              <Link href={'/kategori/' + slug + '/' + city} className="mt-3 inline-block text-sm text-indigo-400">{cityName} geneline bak</Link>
            </div>
          ) : (
