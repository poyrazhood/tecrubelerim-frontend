...
export default function MuhtarBasvuruPage() {
  ...
  const handleSubmit = async () => {
    if (!form.neighborhood || !form.district || !form.city || !form.reason) {
      setError('Tüm alanlar zorunlu'); return
    }
    if (form.reason.length < 50) { setError('Başvuru nedeniniz en az 50 karakter olmalı'); return }
    ...
    if (!res.ok) { setError(d.error || 'Bir hata oluştu'); setSubmitting(false); return }
    ...
    <AppLayout>
      <div className="max-w-lg mx-auto px-4 py-6 space-y-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-black text-lg text-white">Muhtar Başvurusu</h1>
            <p className="text-xs text-white/35">Mahallenizin muhtarı olun</p>
          </div>
        </div>
        <div className="rounded-2xl border border-indigo-500/15 bg-indigo-500/[0.05] p-4 space-y-2">
          ...
          <p className="text-xs text-white/40 leading-relaxed">Muhtarlar, mahalleleri hakkında yetkili yorumlar yapabilen ve topluluğa öncülük eden kullanıcılardır. Başvurunuz admin tarafından incelenir.</p>
        ...
        {existing ? (
          <div className={cn('rounded-2xl border p-4 text-center space-y-2', existing.status === 'PENDING' ? 'border-amber-500/20 bg-amber-500/[0.05]' : existing.status === 'APPROVED' ? 'border-emerald-500/20 bg-emerald-500/[0.05]' : 'border-red-500/20 bg-red-500/[0.05]')}>
            <div className={cn('font-bold text-sm', existing.status === 'PENDING' ? 'text-amber-400' : existing.status === 'APPROVED' ? 'text-emerald-400' : 'text-red-400')}>
              {existing.status === 'PENDING' ? 'Başvurunuz İnceleniyor' : existing.status === 'APPROVED' ? 'Başvurunuz Onaylandı!' : 'Başvurunuz Reddedildi'}
            </div>
          ...
          <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/[0.05] p-6 text-center space-y-2">
            ...
            <div className="font-bold text-emerald-400">Başvurunuz Alındı!</div>
            <div className="text-xs text-white/40">Admin incelemesinin ardından bildirim gönderilecek.</div>
          </div>
        ) : (
          <div className="rounded-2xl border border-white/[0.07] bg-white/[0.02] p-4 space-y-3">
            {[{key:'neighborhood',label:'Mahalle',placeholder:'Bağlarbaşı Mahallesi'},{key:'district',label:'İlçe',placeholder:'Üsküdar'},{key:'city',label:'Şehir',placeholder:'İstanbul'}].map(({key,label,placeholder}) => (
              <div key={key}>
            ...
            <div>
              <label className="text-xs text-white/40 mb-1 block">Başvuru Nedeni ({form.reason.length}/50 min)</label>
              <textarea value={form.reason} onChange={e => setForm(f => ({...f, reason: e.target.value}))} placeholder="Bu mahallede yaşıyorum, işletmeleri yakından tanıyorum..." rows={4}
