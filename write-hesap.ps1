$dir = "C:\Users\PC\Desktop\tecrubelerim-frontend\app\profil\hesap"
New-Item -ItemType Directory -Force -Path $dir | Out-Null
Set-Content "$dir\page.tsx" -Encoding UTF8 -Value @'
'use client'
        ...
        <div className="px-4 pt-5 space-y-5">
          ...
          <div className="rounded-2xl border border-white/[0.07] bg-surface-2 p-4">
            ...
            <Field label="E-posta" icon={Mail}>
              <div className="w-full bg-white/[0.03] border border-white/[0.06] rounded-xl px-3 py-2.5 text-sm text-white/40">
                {user.email}
                {(user as any).emailVerified && <span className="ml-2 text-xs text-emerald-400">✓ Doğrulandı</span>}
              </div>
            </Field>
...
'@
Write-Host "hesap/page.tsx yazildi"
